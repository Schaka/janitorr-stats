...
/**
 * Video files from Blender Foundation's official download server.
 * BigBuckBunny_320x180.mp4 is ~62 MB and ~9 minutes — suitable for real stream testing.
 */
...
class MediaLibrarySetup(private val mediaRoot: Path) {
    ...
    /**
     * Downloads video files to cache if missing, then recreates the full directory/symlink
     * structure that Jellyfin will scan. The Movies and TV Shows directories are wiped on
     * each call so stale entries from previous runs with different naming do not accumulate.
     */
    fun prepare() {
